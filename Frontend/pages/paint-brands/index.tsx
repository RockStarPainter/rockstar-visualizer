...
const PaintSelectionPage = () => {
  ...
  const [selectedPaints, setSelectedPaints] = useState<any[]>([]);
  ...
  const handlePaintSelection = (paint: any) => {
    if (selectedPaints.some((p) => p.code === paint.code)) {
      setSelectedPaints(selectedPaints.filter((p) => p.code !== paint.code));
    } else {
      ...
      <Col key={index} xs={3} className="mb-4">
        <Card
          className={`p-2 paint-card position-relative ${
            selectedPaints.some((p) => p.code === paint.code) ? "selected" : ""
          }`}
        ...
        >
          ...
          {selectedPaints.some((p) => p.code === paint.code) && (
            <FaCheckCircle
              size={24}
              color="black"
              style={{
              ...
              {selectedPaints.map((paint, index) => (
                ...
                >
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setSelectedPaints(
                        selectedPaints.filter((p) => p.code !== paint.code)
                      )
                    }
                  >
                    &times;
